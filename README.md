...
- **免費方案**：使用者選擇「盡量找免費的」，所以接的是 Google Gemini 的免費額度（`gemini-flash-lite-latest`，
...
- **真正的病因其實不是額度（2026-07 追查後修正）**：改成一天一次之後，`HTTP 429` 還是持續出現，一度以為是
  Google 悄悄調緊了免費額度（有查到類似回報），請使用者自己到 Google AI Studio 用量頁面確認，當時看起來像是
  除錯期間短時間內累積太多呼叫造成的一次性尖峰，觀察後成功率恢復到 100%，以為問題已經解決。後來 429 又持續
  出現，進一步查證才發現真正原因：原本呼叫的 `gemini-2.0-flash` 已經在 2026-06-01 被 Google 正式下架停用，
  之後每一次呼叫都會失敗（用 `HTTP 429` 的形式呈現，不是預期中「模型不存在」的 404），跟「額度用完」的錯誤
  長得一模一樣，才會被誤判追查了好幾輪。
- **換成固定日期版本名稱只是把同一個問題延後（2026-07，再次修正）**：第一次修正把模型換成當時最新的
  `gemini-2.5-flash-lite`，結果上線後 `detail` 馬上變成 `HTTP 404`——查證時發現 Google 光是 2026 年內就已經
  接連推出 3.1、3.5、3.6 版本的 Flash-Lite，舊的固定版本名稱淘汰速度比預期快很多。這次改用 Google 官方提供、
  專門為了避免這個問題而設計的**滾動別名** `gemini-flash-lite-latest`——這個名稱本身永遠指向「目前」的輕量版
  模型，由 Google 那邊定期熱切換（若有重大變動會提前至少兩週通知），程式碼這邊理論上不用再因為例行版本更新
...
- **改用 `-latest` 別名之後，呼叫本身成功了，但回傳內容解析失敗（2026-07，第三次修正）**：換成
  ...
  時漏掉後半段。
- **同一個錯誤訊息又出現，代表上一次修正只猜對一部分（2026-07，第四次修正，改成加上診斷而不是再猜一次）**：
  拆掉 Markdown 包裝上線之後，隔天真正跑到的那一次呼叫，`detail` 還是同一句 `Gemini response wasn't in the
  expected format`——這個 sandbox 環境完全連不到 Gemini 正式 API（見上方「一天只呼叫一次」的限制，加上這裡
  沒有一般網路對外連線），沒辦法直接重現失敗的回應內容來精準診斷，前三次修正都只能憑公開文件、社群回報去猜
  最可能的原因（模型下架 → 模型版本又下架 → Markdown 包裝），每次都只解決了一部分。與其再猜第四次，這次改成
  幫程式加上「看得到但不外洩」的診斷：`_call_gemini()` 如果 Gemini 回傳的 `finishReason` 不是 `STOP`（例如
  `MAX_TOKENS`，代表回應在算到一半被截斷），會記錄到伺服器端日誌；`_parse_picks()` 解析失敗時，也會把實際
  收到的原始內容（截斷到安全長度）記錄到日誌——這些都只會出現在 GitHub Actions 的執行紀錄裡，**絕對不會**
  進到公開的 `signals_latest.json`（`detail` 欄位維持原本的安全分類訊息，不會變成外洩管道）。下次這個功能
  又失敗時，就能直接去看真正的回應內容，對症下藥，而不是繼續憑空猜測。
...
3. **部位大小依信心度分配**：不再是不管信心度高低都平均分配資金，而是信心度越高、分配到的虛擬資金越多（仍然單一
   標的上限是起始資金的 10%，避免單押）。
4. **訊號反轉需連續確認才平倉（2026-07 新增，`FLIP_CONFIRMATION_TICKS`，目前只套用在 24小時自動跟單）**：使用者
   回報「最近還是一直虧損」，質疑是不是程式哪裡有問題。實際稽核 24小時帳戶的真實交易紀錄（而不是憑感覺回答）發現：
   最近 103 筆平倉紀錄裡，有 58 筆（超過一半）是因為「多代理訊號又反轉方向」被迫平倉——不是碰到停損也不是碰到
   停利——而這 58 筆的勝率只有 40%，平均每筆是虧損的，是整個帳戶虧損的最大來源；同一段期間裡完全沒有任何一筆
   走到停利目標，只有 3 筆真正走到停損（但每筆虧損都不小）。換句話說：原本設計的「賺 2 賠 1」風報比幾乎沒有機會
   發揮，因為部位常常在還沒走到停損/停利之前，就因為訊號在多空臨界點附近來回擺盪而被迫提前出場，每次「改變心意」
   都要付一次代價（whipsaw）。修正方式不是「讓程式學會預測漲跌」——沒有任何系統能保證預測股價方向，這是金融市場
   本質上的限制，不是工程問題——而是讓系統不要對「單一次」反向訊號反應過度：現在要求新方向（或轉為 HOLD）連續
   出現滿 `FLIP_CONFIRMATION_TICKS`（目前設 2）次之後，才會真的平倉/反手，而不是一有反向訊號就馬上動作，讓部位
   多一點機會撐到真正的停損/停利價位，減少來回抽單的耗損。這個限制目前只套用在 24小時自動跟單
   （`src/pipeline/auto_trader.py`），瀏覽器端 `paperAutoTradeTick()` 尚未套用。
5. **同族群集中度上限（2026-07 修正，瀏覽器端 `paperAutoTradeTick()`）**：使用者回報瀏覽器模擬帳戶單一次自動跟單
