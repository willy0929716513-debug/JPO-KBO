...
- **免費方案**：使用者選擇「盡量找免費的」，所以接的是 Google Gemini 的免費額度（`gemini-2.5-flash-lite`，
...
- **真正的病因其實不是額度（2026-07 追查後修正）**：改成一天一次之後，`HTTP 429` 還是持續出現，一度以為是
  ...
  長得一模一樣，才會被誤判追查了好幾輪。修正方式是把模型換成目前免費方案額度最寬鬆、也還沒被列入下架時程的
  `gemini-2.5-flash-lite`（每天 1,000 次請求，比一般 `gemini-2.5-flash` 的 250 次更寬裕）。這個教訓：往後如果
  這個功能又開始一直出現 429，第一件事應該是先查 https://ai.google.dev/gemini-api/docs/deprecations 目前用的
  模型是否又被下架，而不是預設又是額度問題。
