...
修正方式：新增 `TaiwanHolidayProvider`，向證交所公開的休市時間表查詢今天是否被列為休市（每天最多查一次，有快取，
不會每 5 分鐘都打一次 API）。只有在明確查到「今天有休市紀錄」時才會把原本判斷為開盤的結果改成休市，查詢失敗
（網路問題、格式跟預期不同等）一律當作「沒有額外休市資訊」，安全回退到原本的星期+時間判斷——**絕對不會**因為
這個新檢查而把原本真的有開盤的日子誤判成休市。這個 repo 的開發環境本身連不到一般外部網站，沒辦法在上線前實際
驗證證交所這個端點的即時回應格式，所以採用寬鬆的關鍵字比對（在同一行找到今天的日期加上「休市」字樣）而不是
假設特定的 JSON 欄位名稱，降低格式跟預期不同時解析出錯的風險。如果你之後又遇到台股臨時休市卻沒被正確偵測到，
麻煩跟我說一聲。
