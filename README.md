...
能在使用者沒開網頁時執行程式碼。這個新模組解決了這個限制：把同一套自動跟單規則（每次有做多/做空建議就跟、虛擬資金
平均分配給當下所有建議、碰到停損停利自動平倉）搬進已經每 ~5 分鐘自動執行一次的 GitHub Actions pipeline
...
所以不管你有沒有開著網頁，這個帳戶都會持續前進。`paper.html` 上方的「🤖 24小時自動跟單」區塊只是單純把這個檔案抓下來
顯示，前端完全不會寫入這個帳戶，是唯讀的。這個帳戶跟瀏覽器裡的手動模擬/自動跟單完全獨立，起始資金各自都是 1,000 萬，
兩邊資金互不影響、互不共用。

## 瀏覽器練習帳戶 `docs/paper.html`（純前端，需要開著網頁才會動作）

除了上面的 24小時自動帳戶，頁面下半部還保留一個純瀏覽器端的練習帳戶，讓你可以用一筆虛擬台幣（預設 1,000 萬）
...
- **自動跟單**：在 `paper.html` 頁面上有一個開關，打開後系統每次更新建議清單時，會自動幫你模擬開倉（固定約 100 萬台幣
  一筆，即起始資金的 10%）/ 平倉，完全依照卡片上顯示的做多/做空/觀望訊號，不需要手動操作；自動跟單開的倉位跟手動開的倉位互不干擾。
  候選標的會依市場類別（台股/美股/期貨商品/外匯/加密貨幣）輪流開倉，避免台股觀察清單過長，把虛擬資金全部用在台股上。
